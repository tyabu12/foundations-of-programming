...
(* inserts_ekikan : t -> ekikan_t list -> t *)
let inserts_ekikan tree ekikan_lst =
  (* 目的: ekikan_tree_t 型の木 tree と駅間 ekikan を受け取り,
          その情報を挿入した木を返す *)
  (* insert_ekikan : ((string * (string * float)) list) t -> ekikan_t -> t *)
  let insert_ekikan tree ekikan =
    let lst =
      try search tree ekikan.kiten
      with Not_found -> []
    in
    insert
      (insert
        tree
        ekikan.kiten
        ((ekikan.shuten, ekikan.kyori) :: lst))
      ekikan.shuten
      ((ekikan.kiten, ekikan.kyori) :: lst)
  in
...
(* get_ekikan_kyori : string -> string -> t -> float *)
...
let dijkstra shiten_romaji syuten_romaji =
  ...
  (* koushin : eki_t -> eki_t list -> ((string * string, float) list) t -> eki_t list *)
  ...
  (* dijkstra_main : eki_t list -> ((string * (string * float)) list) t -> eki_t list *)
  let rec dijkstra_main v ekikan_lst = match v with
    | [] -> []
    | hd :: tl ->
      let (p, v') = saitan_wo_bunri hd tl in
...
let () =
  ...
  time (fun _ -> let _ = dijkstra "ikebukuro" "iidabashi" in ()) "dijkstra (RedBlack Tree)";
  ()
