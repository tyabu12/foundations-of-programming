...
let rec saitan_wo_bunri l =
  let p0 = ({namae = ""; saitan_kyori = infinity; temae_list = []}, []) in
  List.fold_right (fun eki p ->
    let (min_eki, l') = p in
    if min_eki.namae = "" then (eki, [])
    else if eki.saitan_kyori < min_eki.saitan_kyori then (eki, min_eki :: l')
                                                    else (min_eki, eki :: l')
  ) l p0
